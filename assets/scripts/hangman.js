...
let fullText, hintText, numWrong, unstable1Timer, unstable2Timer, unstable3Timer, unstable4Timer, phraseArray;
...
function layoutWord(){
    ...
    //set up array for the word and the hint
    ...
    document.onkeypress = function(evt){
        // checks if the instruction panel is visible. If it is, it exits the function
        let mod = document.getElementById("inst");
        if(mod.style.display !="none" && mod.style.display !=""){
            return;
        }
        //gets the value of the letter typed - checks if has already been typed - if yes exit function (Prevents punishing the user for retyping the same letter)
        let theLetter = evt.key.toLowerCase();
        if(chosenLetters.indexOf(theLetter) > -1){
            return;
        }else{
            //adds the letter to the array of chosen letters
            chosenLetters.push(theLetter);
        }

        //checks to see it the key pressed is actually a letter - if yes it checks to see if the letter exists in the final word/phrase
        if(evt.code.toString().includes("Key")){
            let isGoodLetter = theWord.toLowerCase().includes(theLetter);
            if(isGoodLetter){
                //executes the command to add the letter in the spans in th ephrase holder
                placeLetter(theLetter);
            }else{
                //executes the command to add the letter to the missed letter array
...
function placeLetter(l){
    ...
    let letters = holder.getElementsByTagName("span");           
...
function addToMisses(l){
    let letter = this.document.createElement("span");     
...
function addUnstable(){
    ...
    if(numWrong == 5){
        ...
        let spans = $("#wordShow span");
        //loops through the word and looks for missing letter - added letter and class to turn the color to red
